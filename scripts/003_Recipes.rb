  ...
  module Recipes
    ...
    def normalize_condition(condition)
      if condition[:operator] || condition['operator']
        ...
        # --- Pokemon condition fields ---
        # Species
        db_sym = condition[:db_symbol] || condition['db_symbol']
        normalized[:db_symbol] = db_sym.to_sym if db_sym

        # Minimum loyalty/happiness (0–255)
        min_loyalty = condition[:min_loyalty] || condition['min_loyalty']
        normalized[:min_loyalty] = min_loyalty.to_i if min_loyalty

        # Minimum level
        min_level = condition[:min_level] || condition['min_level']
        normalized[:min_level] = min_level.to_i if min_level

        # Move known by the Pokemon
        move_sym = condition[:move] || condition['move']
        normalized[:move] = move_sym.to_sym if move_sym

        # Ability
        ability_sym = condition[:ability] || condition['ability']
        normalized[:ability] = ability_sym.to_sym if ability_sym

        # Held item
        holding_item = condition[:holding_item] || condition['holding_item']
        normalized[:holding_item] = holding_item.to_sym if holding_item

        # Gender (0 = genderless, 1 = male, 2 = female)
        gender = condition[:gender] || condition['gender']
        normalized[:gender] = gender.to_i if gender

        # Form index — use key? to preserve form: 0
        form = condition.key?(:form) ? condition[:form] : condition['form']
        normalized[:form] = form.to_i unless form.nil?

        # Shiny — use key? to preserve shiny: false
        shiny = condition.key?(:shiny) ? condition[:shiny] : condition['shiny']
        normalized[:shiny] = shiny unless shiny.nil?
